...
versus using the efficient but not as stable explicit Runge-Kutta methods applied by MATLAB's \texttt{ode45()}.
...
\begin{align*}
	h^0 &: x=x \\
	h^1 &: f(x) = f(x) \\
	h^2 &: d_2(x) = f_2(x) + f'(x)f(x) \\
	h^3 &: d_3(x) = f_3(x) + f'(x)f_2(x) + f'_2(x)f(x) + f''(x)f'(x)f(x) + f'(x)f'(x)f(x) \\
	& \vdots
\end{align*}
...
% closeness of perturbed hamiltonian
