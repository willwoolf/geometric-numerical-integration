...
\begin{figure}
    ...
    \caption{
        ...
        The left figure uses the "ES2" method given by Equation \ref{eqn:secondorderstrangsplittingmethod} taking the values for $x$.
...
An alternative method suggested in \cite{blanes_pos_2022} is the second order Magnus integrator, for which the autonomous version is given by
\begin{equation}
    x_{n+1} = \exp\left(h A \left( \exp\left(\frac{1}{2}h A(x_n)\right) x_n \right) \right) x_n.
    \label{eqn:secondordermagnus}
\end{equation}
The Magnus scheme is given as a generalised structure which can be used to construct higher order methods.
% generalised m stage magnus integrator

%% truncation error for second order magnus
\begin{equation*}
    \begin{aligned}
